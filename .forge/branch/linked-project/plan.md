...
### T21 — Délégation — branche du même dépôt (`SCOPE: branch`) et `FORGE_QUESTION` porteur du contenu entier
**Effort:** M
**Files:** `skills/forge/SKILL.md`, `skills/forge/phases/p2-brief.md`, `skills/forge/phases/p4-plan.md`, `skills/forge/phases/p5-resume.md`, `README.md`, `README.fr.md`, `CHANGELOG.md`
**Description:** (1) Commande parent — nouvelle section « Délégation — branche du même dépôt » dans `p5-resume.md`, au même rang que « Délégation — projet lié ». Déclencheur : demande de lancer un agent ou un forge délégué sur une branche X du dépôt courant — « lance un agent forge sur la branche X », « lance un agent sur la branche X », « lance un forge délégué sur X », « ouvre la branche X dans un agent » et toute formulation équivalente (agent / forge délégué + nom de branche, sans chemin de dossier) ; un chemin de dossier cité → « Délégation — projet lié ». Gardes : X = `<BRANCH>` → "Already on `X`." STOP ; X absente (`git rev-parse --verify --quiet refs/heads/X`) → "Branch `X` does not exist — create it first." STOP, jamais créée à la place de l'utilisateur. Worktree : `git worktree list --porcelain` — worktree existant pour X réutilisé (`reuse`) ; sinon `git worktree add <dossier-du-dépôt>-<X> X`, dossier frère du dépôt (`create`). Tableau récapitulatif (trois colonnes ; lignes `branch`, `worktree` → chemin + `reuse` / `create`, `agent`), puis confirmation unique `AskUserQuestion` — `header` : `Delegate`, options `Open the branch` / `Cancel`. Aucune commande git avant cette confirmation ; `Cancel` → STOP. Sur `Open the branch` : worktree créé si besoin (échec git → erreur telle quelle, STOP) ; sous-agent en tâche de fond, prompt figé `FORGE_DELEGATED` (ROOT = chemin absolu du worktree, BRANCH = X, LANGUAGE) + ligne `SCOPE: branch` ; le prompt du projet lié reçoit `SCOPE: linked`. Boucle de relais identique. Retour `FORGE_DONE` en `SCOPE: branch` : aucune écriture dans le plan ni le log parent ; compte rendu d'une ligne ; worktree laissé en place avec rappel "`git worktree remove <chemin>` once the branch is engraved." (2) `SKILL.md`, « Mode délégué » — deux portées. Condition : prompt `FORGE_DELEGATED` seul ; ligne `SCOPE:` fixe la portée, `linked` par défaut (ligne absente = linked). `SCOPE: linked` : comportement actuel — mandat, `## Origin`, plan validé vaut accord, enchaînement sans question de mode, `FORGE_DONE` avec `done` / `blocked` / `out_of_mandate`. `SCOPE: branch` : pas de `## Origin` ; lecture de project / standards / brief / log / plan telle que les phases la prescrivent ; chaque choix relayé par `FORGE_QUESTION` — objectif du brief, validation de plan, mode (enchaîner / choisir), feu vert avant chaque ligne de code, `grave` — le raccourci « plan validé vaut accord » ne s'applique pas ; `FORGE_DONE` réduit à un compte rendu d'une ligne (`done` / `blocked` sans `← parent`, `out_of_mandate: none`). (3) `FORGE_QUESTION` porteur du contenu entier — les deux portées. Champ `content:` (bloc multiligne entre `question:` et `options:`) : toute question demandant le démarrage d'une tâche ou la validation d'un contenu présenté — description complète de la tâche, plan complet, objectif du brief, entrée `## Scope & rules`, rapport — y recopie ce contenu intégralement, jamais un résumé ; sans contenu à valider → `content: none`. Boucle de relais : le parent affiche `content` tel quel à l'utilisateur, puis pose l'`AskUserQuestion` avec `header` et options recopiés. Règle unique dans `SKILL.md`, renvoi depuis p2 / p4 / p5. (4) Phases. `p2-brief.md` : question d'objectif (`options: none`) et validation → `FORGE_QUESTION` en mode délégué. `p4-plan.md` : garde `## Origin` requalifiée « mandat » (`SCOPE: linked`), renvoi `FORGE_QUESTION` (approches, validation du plan avec le plan entier en `content`) étendu à tout mode délégué. `p5-resume.md` étapes 7 et 8 : `SCOPE: linked` inchangé ; `SCOPE: branch` → « Nothing open — what do we do next? » et question `Mode` via `FORGE_QUESTION` ; démarrage de chaque tâche → `FORGE_QUESTION` avec la description entière en `content`. « Livraison » en `SCOPE: branch` : git sous ROOT (le worktree), tableau récapitulatif en `content`, confirmation `Engrave` relayée. (5) README en/fr : section « Delegating to a branch of the same repository » sous celle du projet lié, même plan ; paragraphe « relayed questions carry the full content » dans la section du projet lié. CHANGELOG : `Added` (délégation par branche, worktree) et `Changed` (`FORGE_QUESTION` porteur du contenu entier, `SCOPE`) sous Unreleased, puis section `[0.11.0] — 2026-09-20` et liens de comparaison.
[x] SCOPE linked/branch in SKILL.md, content field in FORGE_QUESTION, branch-delegation section + worktree in p5, p2/p4 guards, README en/fr, CHANGELOG 0.10.1 (renamed from 0.11.0 at engrave: patch release)

### T22 — Branche du projet lié nommée `<parent>/<BRANCH>`
**Effort:** S
**Files:** `skills/forge/phases/p5-resume.md`, `skills/forge/SKILL.md`, `README.md`, `README.fr.md`, `CHANGELOG.md`
