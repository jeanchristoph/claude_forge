...
- Jamais de gabarit de fichier de sortie figé en dur (fence avec libellés littéraux type `## Objectif`, `**Effort :**`) — le texte fixe la langue du gabarit indépendamment de celle de l'utilisateur. Décrire à la place la structure attendue par instruction (sections requises, ordre, rôle et contenu de chacune) ; Claude génère les libellés dans la langue de l'utilisateur au moment de l'écriture.
...
- Terme du domaine du skill (Brief, Plan, État, Décisions & Contraintes, Hors périmètre, Historisation) → toujours le même mot une fois introduit, jamais de synonyme.
